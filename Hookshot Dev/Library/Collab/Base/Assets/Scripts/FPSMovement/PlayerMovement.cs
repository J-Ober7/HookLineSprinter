...
public class PlayerMovement : MonoBehaviour {
    ...
    {
        RaycastHit hit;
        if(Physics.Raycast(transform.position, Vector3.up, out hit, .8f, LayerMask.NameToLayer("Ground")) && !hit.collider.isTrigger)
        {
            return false;
        }
    ...
    private void Momentum()
    {
        ...
        {
            currentMaxSpeed = Mathf.Lerp(this.currentMaxSpeed, minMaxSpeed, maxSpeedIncrement* Time.deltaTime);
        }
    ...
    private void Jump() 
    {
        if ((grounded || hook.ReadyToHookJump()) && readyToJump)
        {
            ...
            else
            {
                ...
                rb.AddForce(orientation.forward * jumpForce * .1f * horizontalVelocity.magnitude);
