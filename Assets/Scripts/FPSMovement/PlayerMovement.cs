...
public class PlayerMovement : MonoBehaviour {
    ...
    public float maxExtraGravity = 1300;
    public float coyotteTimeDelay = .23f;
    private bool gravityResetCheck;
    ...
    private void Update() 
    {
        if (levelManager.gameOver == false && levelManager.isPaused == false)
        {
            MyInput();
            Look();
        }
    ...
    private void MyInput() 
    {
        ...
        {
            sensitivity -= 10;
            sensitivity = Mathf.Max(sensitivity, 1);
        }
        if (Input.GetKeyDown(KeyCode.Alpha0))
        {
            sensitivity += 10;
            sensitivity = Mathf.Min(sensitivity, 100);
        }
    ...
    private void Momentum()
    {
        ...
        {
            //Debug.Log("decelerating: " + rb.velocity.magnitude + " < " + currentMaxSpeed * 0.35);
        ...
        {
            rb.velocity = new Vector3(Vector2.ClampMagnitude(rbv, maxMaxSpeed).x, rb.velocity.y, Vector2.ClampMagnitude(rbv, maxMaxSpeed).y);
        }
    ...
    private void Movement() 
    {
        ...
        //Extra gravity
        if(hook.isHooking == true && gravityResetCheck == false)
        {
            currentExtraGravity = extraGravityForce;
        }
        gravityResetCheck = hook.isHooking;

        rb.AddForce(Vector3.down * Time.deltaTime * currentExtraGravity);
        if(currentExtraGravity < maxExtraGravity)
        {

            currentExtraGravity += .5f * extraGravityForce * Time.deltaTime;
        }

        //Debug.Log(currentExtraGravity);
        ...
        if (crouching && grounded && readyToJump && !crouchWalk)
        {
    ...
    private void Look() 
    {
        ...
        if(KillPlaneBehavior.isRespawning)
        {
            xRotation = 0;
            desiredX = 0;
        }
    ...
    private void OnCollisionStay(Collision other) 
    {
        ...
        if (!cancellingGrounded) 
        {
            ...
            Invoke(nameof(StopGrounded), coyotteTimeDelay);
        }
    }
